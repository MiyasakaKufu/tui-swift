...
`<責任者の GitHub ユーザー名>/<issue 番号>-<内容>` とする。
エージェントに作らせたブランチも、作業を指示し PR に責任を持つ人の名前を頭に付ける。
道具の名前を付けると、どの人が責任を持って作ったのかがブランチ名から分からなくなる。

作業を始める前に issue を立て、ブランチ名に番号を入れる。内容だけでは、どの issue に対応する
ブランチなのかが分からない。
