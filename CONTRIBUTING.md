...
なぜその定数なのか、なぜその判定なのかを支える外部仕様・規格の事実は DocC に書く。それが支えている
定数・型・関数に名前を与え、その宣言の DocC に置く。非 public な宣言にも書けるので、対象は public に限らない。

出どころは `- See:` に書き、文書名と URL を併記する。文書名を先に置くのは、リンクが切れても何を指して
いたかが残るため。指す先は節の題名で書く。節に題名が無ければ、定義や表の名前で指す。節番号は使わない。
版で動く。
...
URL は最新版を指すものを選ぶ。版ごとに URL が分かれる規格でも、特定の版を指すと改訂後に古い版を
指したままになる。書いた URL は開けることを確かめる。
...
誘因（読み手がやりたくなる変更）と帰結（それをすると壊れるもの）の両方を書く。誘因が無ければただの
事実説明になり、帰結が無ければ読み手は破ってよいかを判断できない。帰結だけでは信じてもらえないときは、
なぜそうなるかを足してよい。
...
要約の書き出し、コールアウトの選び方、コードを言い換えただけのコメント、`//` が Why not になっているか、
典拠が DocC に置かれ `- See:` の URL が開けるかは機械的に判定できない。レビューで見る。手順は
`.claude/skills/comment-review` にある。
